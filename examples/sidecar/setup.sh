...
# Build cfn-sidecar sidecar image (Istio approach)
echo "Building cfn-sidecar..."
...
kind load docker-image cfn-sidecar:latest --name ${CLUSTER_NAME}
kind load docker-image cfn:latest --name ${CLUSTER_NAME}
...
kubectl rollout status deployment/cfn -n a2a-demo --timeout=120s
...
echo ""
echo "Watch logs:"
echo "  CFN API:    kubectl logs -f -n a2a-demo deployment/cfn"
echo "  Agent A:         kubectl logs -f -n a2a-demo deployment/agent-a -c agent-a"
echo "  Agent A Sidecar: kubectl logs -f -n a2a-demo deployment/agent-a -c cfn-sidecar"
echo "  Agent B:         kubectl logs -f -n a2a-demo deployment/agent-b -c agent-b"
echo "  Agent B Sidecar: kubectl logs -f -n a2a-demo deployment/agent-b -c cfn-sidecar"
echo ""
echo "Cleanup:"
